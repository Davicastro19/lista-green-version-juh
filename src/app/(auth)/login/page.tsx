...
export default function SignIn() {
  ...
  const { onSubmit, handleSubmit, register, errors, isSubmitting } = useLoginForm();
...
<div>
      ...
      <Card className="w-full max-w-md px-1  py-6 shadow-2xl rounded-lg bg-white border-0">
        <CardContent>
        <button className="bg-transparent cursor-pointer hidden flex-row justify-center items-center  text-[var(--darkgreenhome)] " onClick={()=> router.push('/')}>
                          <ChevronLeftIcon size={40} className="text-[var(--darkgreenhome)]" aria-label="Voltar" />

                        </button>
          <h2 className="text-2xl font-bold text-center text-[var(--darkgreenhome)]">Bem-vindo de volta!</h2>
          <p className="text-center text-gray-500">Faça login para acessar a Lista Green.</p>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 mt-6">
            ...
            <div className="flex justify-between items-center text-sm">
              <Link href="/forgot-password" className="text-[var(--darkgreenhome)] hover:underline">Esqueci a senha</Link>
            </div>
          ...
          <div className="text-center mt-4 text-sm">
            <span className='text-gray-500'>Não tem conta ainda? </span>
            <Link href="/cadastro" className="text-[var(--darkgreenhome)] font-semibold hover:underline">Registrar-se</Link>
          </div>
        </CardContent>
      </Card>
      </div>
