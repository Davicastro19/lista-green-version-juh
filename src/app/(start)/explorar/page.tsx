...
export default function Explore() {
  ...
  const { data, error, isLoading } = useFetch<{
    ...
    pageSize: number,
    totalPages: number
  }>(`/explore?page=${page}&pageSize=6&search=${query}`);
  ...
  if (error || !data) {
    return <div className="text-red-500 text-center">Erro ao carregar</div>;
  }
  ...
  return (
    <>
      <h1 className="text-2xl font-bold text-center my-5 mt-2">Explore aqui</h1>

      <div className="flex justify-center mb-5">
        <input
          type="text"
          placeholder="Busque pela palavra ..."
          value={search}
      ...
      <div className="flex flex-wrap space-y-10 justify-around w-full">
        {articles.map((article: IArticleData) => (
          <ArticleCard key={article.slug} data={article} type='ww' />
        ))}
      </div>
      {articles.length > 0 ?
        (
          <div className="flex justify-center mt-5 mb-20 space-x-2 items-center">
            ...
            <text className="ml-2 px-3 py-1  hover:bg-[var(--basegreen)] text-[var(--darkgreen)]  rounded-lg">Nada encontrado</text></div>
        )}
    </>
  );
}
